...
Auth is automatic inside Databricks Apps (service principal OAuth). For local dev
the SDK falls back to a DEFAULT profile / env vars (DATABRICKS_HOST + token).
"""
...
class SqlClient:
    ...
    def execute(self, statement: str, parameters: list | None = None) -> list[dict]:
        ...
        `parameters` is a list of databricks.sdk.service.sql.StatementParameterListItem
        for parameterized queries (prevents SQL injection on user-supplied filenames).
        """
        ...
        # Poll if still running (large queries may exceed wait_timeout).
